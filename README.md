...
Este manual describe el paso a paso para utilizar el programa y explica de forma detallada su funcionamiento interno. Su formato con sintaxis Markdown nativa lo hace ideal para servir como presentación en **GitHub**.
...
> **📁 Sobre las capturas de imagen:**
> Para almacenar las imágenes que complementarán este manual en GitHub, crea una carpeta llamada `assets` dentro de esta misma carpeta del proyecto (al lado de `app.py`).

![Selección de Tamaño](assets/1_seleccion_tamano.png)
*(**Nota**: Toma captura de la ventana hasta el sector de la selección del tamaño. Guárdala en la carpeta generada como `assets/1_seleccion_tamano.png`)*
...
*(**Nota**: Guarda una captura mostrando la matriz con datos digitados como `assets/2_ingreso_datos.png`)*
...
*(**Nota**: Toma una captura amplia donde se observe la alerta verde con la solución y la apertura del panel del paso a paso; luego guárdala como `assets/3_solucion_pasos.png`)*
...
En el algoritmo original de un sistema $Ax=b$, se busca fragmentar la matriz de coeficientes $A$ en el producto de dos matrices especiales: $L$ (una matriz triangular *inferior* con su diagonal dominada por 1s) y $U$ (una matriz triangular *superior*).
...
* **Generación y Eliminación:** Ejecuta las reglas de Doolittle operando iterativamente y reduciendo filas por coeficientes en los triángulos inversos. Si $U$ es nula en su diagonal para esa etapa y no se puede pivotar más, detecta el determinante muerto identificando matrices matemáticamente "*singulares*" y retornando un vector de error limpio que neutraliza un quiebre de hardware en la computadora.
